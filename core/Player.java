...
public class Player extends GameObject implements Gravity{
	...
	private void breakAtMouse(int x, int y){
		if(Block.getBlock(x, y) == null) return;
	...
	private boolean placeAtMouse(int x, int y) {
		if(getSelectedSlot().getType().equals("empty") || getSelectedSlot() instanceof Tool) return false;
		if(Block.getBlock(x+30, y) != null || Block.getBlock(x, y+30) != null || Block.getBlock(x, y-30) != null || Block.getBlock(x-30, y) != null){
			if(x > Driver.WIDTH/2 - REACH && x < Driver.WIDTH/2 + REACH + width){
				if(y > Driver.HEIGHT/2 - REACH && y < Driver.HEIGHT/2 + REACH + height){
					if(Driver.getWorld().getLoaded()[(int) ((x-Driver.getX())/30)][(int) ((y-Driver.getY())/30)] == null){
						Driver.getWorld().getLoaded()[(int) ((x-Driver.getX())/30)][(int) ((y-Driver.getY())/30)] = new Block(x-Driver.getX() - (x-Driver.getX())%30, y-Driver.getY() - (y-Driver.getY())%30, getSelectedSlot().getType());
	...
	private boolean canMoveRight() {
		return !(Driver.getWorld().isOnMap(this.getX()+this.getWidth()+1,this.getY()) || Driver.getWorld().isOnMap(this.getX()+this.getWidth()+2,this.getY() + this.getHeight()/2) || Driver.getWorld().isOnMap(this.getX()+this.getWidth()+2,this.getY() + this.getHeight()-10));
	}

	private boolean canMoveLeft() {
		return !(Driver.getWorld().isOnMap(this.getX()-1,this.getY()) || Driver.getWorld().isOnMap(this.getX()-1,this.getY() + this.getHeight()/2) || Driver.getWorld().isOnMap(this.getX()-1,this.getY() + this.getHeight()-10));
	}

	private boolean canMoveUp(int vel) {
		return !(Driver.getWorld().isOnMap(this.getX()+1,this.getY()+this.height + vel) || Driver.getWorld().isOnMap(this.getX()+this.getWidth()-1,this.getY()+this.height + vel) || Driver.getWorld().isOnMap(this.getX()+width/2,this.getY()+this.height + vel));
	}

	private boolean isInGround(){
		return (Driver.getWorld().isOnMap(xPos, yPos) || Driver.getWorld().isOnMap(xPos+width/2, yPos) || Driver.getWorld().isOnMap(xPos + width-1, yPos))
				|| (Driver.getWorld().isOnMap(xPos, yPos + height/2) || Driver.getWorld().isOnMap(xPos+width/2, yPos + height/2) || Driver.getWorld().isOnMap(xPos + width-1, yPos + height/2))
				|| (Driver.getWorld().isOnMap(xPos, yPos + height) || Driver.getWorld().isOnMap(xPos+width/2, yPos + height) || Driver.getWorld().isOnMap(xPos + width-1, yPos + height));
	}
	...
	private boolean isHeadInGround(){
		return (Driver.getWorld().isOnMap(xPos, yPos + height) || Driver.getWorld().isOnMap(xPos+width/2, yPos + height) || Driver.getWorld().isOnMap(xPos + width-1, yPos + height));
	}
	...
	private boolean isPlayer(int x, int y, int w, int h){
		...
		if(x > xCenter && x < xCenter + width && y > yCenter && y < yCenter + height) return true;
		if(x > xCenter + w && x < xCenter + w + width && y > yCenter && y < yCenter + height) return true;
		if(x > xCenter + w && x < xCenter + width + w && y > yCenter + h && y < yCenter + height + h) return true;
		if(x > xCenter && x < xCenter + width && y > yCenter + h && y < yCenter + height + h) return true;
		return false;
	}
